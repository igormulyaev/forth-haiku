...
function optimize(code, result_limit) {
  ...
  for (var i = 0; i < code.length; i++) {
    // console.log('mul_src: ' + code[i]);
    for (;;) {
      if (code[i].search(/dstack\.pop\(\)/) >= 0) {
        if (dstack.length === 0) {
          console.log('mul_err2\n' + code[i]);
          return BOGUS;
        }
        var tmp = dstack.pop();
    ...
    if (code[i] === '} else {') {
      if (cstack.length === 0) {
        console.log('mul_err4\n');
        return BOGUS;
      }
      var frame = cstack.pop();
      if (frame[0] !== 0) {
        console.log('mul_err5\n');
        return BOGUS;
      }
    ...
    if (code[i] === '}') {
      if (cstack.length === 0) {
        console.log('mul_err6\n');
        return BOGUS;
      }
      var frame = cstack.pop();
      if (dstack.length != frame[1].length ||
          rstack.length != frame[2].length) {
        console.log('mul_err7\n');
        return BOGUS;
      }
      var decls = '';
      var fixup1 = '';
    ...
    // console.log('mul_dst: ' + code[i]);
  }

  if (rstack.length !== 0) {
    console.log('mul_err8\n');
    return BOGUS;
  }
  if (dstack.length > 4) {
    console.log('mul_err9\n');
    return BOGUS;
  }
...
function compile(src_code) {
  ...
  console.log('mul_code_length: ' + code.length);
  if (code.length > 16000) {
    console.log('mul_err13\n');
    return BOGUS;
  }
